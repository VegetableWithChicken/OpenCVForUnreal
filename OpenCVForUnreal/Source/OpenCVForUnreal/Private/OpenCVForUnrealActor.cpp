...
TSharedPtr<FJsonObject> AOpenCVForUnrealActor::GetJsonObjectFromFile(FString FileDir, FString FileName)
{
...
void AOpenCVForUnrealActor::BeginPlay()
{
	/**读取配置文件配置信息*/
	FString PluginConfigDir = FPaths::ProjectPluginsDir() + "OpenCVForUnreal/Config/";
	PluginConfigDir = FPaths::ConvertRelativePathToFull(PluginConfigDir);
...
// Called every frame
void AOpenCVForUnrealActor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);


}
void AOpenCVForUnrealActor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AOpenCVForUnrealActor::InitCameraAndThreadRunnable(uint32 index)
{
...
void AOpenCVForUnrealActor::ReadFrame()
{
		if (Cap.isOpened())
		{
			cv::Mat frame;
...
void AOpenCVForUnrealActor::EnhanceImgBrightness(cv::Mat& inMat, float inAlpha, float inBrightlessValue)
{
	int rows = inMat.rows;
	int cols = inMat.cols;
...
UTexture2D* AOpenCVForUnrealActor::ConvertMat2Texture2D(const cv::Mat& inMat)
{
	...
	void* Datas = NewTexture->GetPlatformData()->Mips[0].BulkData.Lock(LOCK_READ_WRITE);
	FMemory::Memmove(Datas, out.data, DataSize);
	NewTexture->GetPlatformData()->Mips[0].BulkData.Unlock();
	NewTexture->UpdateResource();
	return NewTexture;
}
void AOpenCVForUnrealActor::CutImage(const cv::Mat inMat, FVector2D inPos)
{
...
void AOpenCVForUnrealActor::CutImageRect(const cv::Mat inMat, cv::Rect inRect)
{
...
void AOpenCVForUnrealActor::face_recognize(const cv::Mat& inMat)
{
...
void  AOpenCVForUnrealActor::DNN_face_recognize(const cv::Mat& inMat)
{
	if (inMat.empty()) return;
...
void AOpenCVForUnrealActor::DNN_Yolo3_ObjectDetection(cv::Mat inMat)
{
...
void AOpenCVForUnrealActor::ScaleRect(cv::Rect& inRect, float scale)
{
...
void AOpenCVForUnrealActor::WhiteFace(cv::Mat inMat, uint32 alpha, uint32 beta)
{
